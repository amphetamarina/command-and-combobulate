...
namespace OpenRA.Mods.Combobulate.Traits
{
	...
	public class CombobulateBridge : IWorldLoaded, ITick, INotifyActorDisposing, IRenderAnnotations
	{
		...
		readonly Dictionary<string, CombobulateTerminalRef> islandRefs = new();
		...
		readonly Dictionary<string, CombobulateLabel> agentLabels = new();
		...
		public void RegisterPlayerTerminal(string id, Actor actor)
		{
			...
			islands[id] = actor;
			islandRefs[id] = actor.TraitOrDefault<CombobulateTerminalRef>();
		}
				...
				if (region.IsTerminal)
				{
					...
					if (!islands.ContainsKey(region.Path))
					{
						...
						if (w.Map.Contains(cell))
						{
							...
							islandRefs[region.Path] = island.TraitOrDefault<CombobulateTerminalRef>();
						}
					}
				}
				else
		...
		List<Actor> BuildFolderWalls(World w, Region region)
		{
			...
			for (var dy = 0; dy < height; dy++)
			{
				for (var dx = 0; dx < width; dx++)
				{
					...
					if (isLabelCorner)
					{
						var label = actor.TraitOrDefault<CombobulateLabel>();
						if (label != null)
							label.Label = name;
					}
		...
		void UpdateTerminalLabel(string terminal, double? fraction, string message)
		{
			...
			var label = island.TraitOrDefault<CombobulateLabel>();
			if (label == null)
				return;
		...
		void ApplyFiles(World w, List<FolderFiles> files)
		{
			...
			foreach (var folder in files)
			{
				...
				foreach (var entry in folder.Entries)
				{
					...
					var label = building.TraitOrDefault<CombobulateLabel>();
					if (label != null)
		...
		void ApplyAgents(World w, List<AgentSnapshot> snapshots)
		{
			...
			foreach (var snap in snapshots)
			{
				...
				if (!agents.TryGetValue(snap.Id, out var unit))
				{
					...
					agentLabels[snap.Id] = unit.TraitOrDefault<CombobulateLabel>();
				}
